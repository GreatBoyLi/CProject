...
void RKey_Add(byte1 sta_matr[4*4],word w[4]){
	for(int i=0;i<4;i++){
		...
		sta_matr[i] = sta_matr[i]^byte1(k0.to_ulong());
		sta_matr[i+4] = sta_matr[i+4]^byte1(k1.to_ulong());
		sta_matr[i+8] = sta_matr[i+8]^byte1(k2.to_ulong()); 
		sta_matr[i+12] = sta_matr[i+12]^byte1(k3.to_ulong());
	} 
...
// 5 加密函数
void encrypt(byte1 sta_matr[4*4],word w[4*(Nr+1)]){
	word key[4];
	for(int i=0; i<4; i++)
		key[i] = w[i];
	//先进行一次轮密钥加 
